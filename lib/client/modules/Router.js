export class Router {
  ...
  _onChanged() {
    ...
    if (this._isAnimatingOut) {
      console.log(' Either animating in or loading still - sorry');
      return Promise.resolve();
    }
    ...
    return this._animateOut().then(() => {
      ...
      console.log(`Async View Resolved With ${this._newContent.getAttribute('route')}`);
  ...
  _boostrapNavigation() {
    ...
    document.querySelectorAll('.header .header-content nav li').forEach(link => {
